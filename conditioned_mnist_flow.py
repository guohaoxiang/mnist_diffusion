...
exp_name = 'flow_ori_net_bs1024_cfg2_ep10'
eval_mode = True
ckpt_path = 'flow_ori_net_bs1024_cfg_ep10.pt'
# ckpt_path = 'flow_ori_net_bs1024_ep10.pt'
...
# Our network 
if not use_cfg:
    net = ClassConditionedUnet(num_classes).to(device)
else:
    net = ClassConditionedUnet(num_classes + 1).to(device)
...
if not eval_mode:
    ...
    for epoch in range(n_epochs):
        for x, y in tqdm(train_dataloader):
            ...
            if use_cfg and random.random() < drop_ratio:
                y = 10 * torch.ones(1024, device = 'cuda:0', dtype = torch.int64)
            else:
                y = y.to(device)
...
y_empty = 10 * torch.ones(80, device = 'cuda:0', dtype = torch.int64)
...
with torch.no_grad():
    for i, t in enumerate(timesteps):
        ...
        if use_cfg:
            noise_pred_uncond = net(latent_model_input, timestep, y_empty)
